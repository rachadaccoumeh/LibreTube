...
open class OnlinePlayerService : AbstractPlayerService() {
    ...
    private fun setStreamSource() {
        val streams = streams ?: run {
            Log.w(TAG(), "setStreamSource: streams is null, cannot set source")
            return
        }
        ...
        when {
            ...
            !streams.isLive && streams.serverAbrStreamingUrl != null && streams.videoPlaybackUstreamerConfig != null -> {
                Log.w(TAG(), "setStreamSource: using SABR for $videoId")
                ...
                Log.w(TAG(), "setStreamSource: SABR media source set for $videoId")
                return
            }
            // DASH
            streams.videoStreams.any { it.url?.startsWith("sabr://") != true } -> {
                Log.w(TAG(), "setStreamSource: using DASH for $videoId")
